...
PROPERTY_LISTEN_SET_VALUE(NLEKey::Render::kRender_Scale, OnSetRenderScale)
PROPERTY_LISTEN_SET_VALUE(NLEKey::Render::kRender_Scale_Enable, OnSetRenderScaleEnable)
...
{	
	...
	TIMELINE_FOR_EACH(it)
	{
		...
		NLE_ASSERT(nTotalFrmCount <= NLECommon::Const::FRAMEINDEX_MAX + 1);
	}
	...
	NLE_ASSERT(rngRender.Count() <= NLECommon::Const::FRAMEINDEX_MAX + 1);
}
...
HRESULT CNLETimelineBase::OnSetRenderFrameRate(LPCWSTR keyName, CNLEValue& ralFramerate)
{
	...
	{
		NLE_ASSERT(FALSE); LOGGER_ERROR_(L"Invalid frame : %.3lf", ralNewFramerate.Value());
		return E_INVALIDARG;
	}
	TIMELINE_FOR_EACH(ppClip)
	{
		...
		if (ralOldFramerate != ralNewFramerate)
		{
			...
			{
				LOGGER_ERROR_(L"Failed to set framerate:%.3lf", ralNewFramerate.Value());  NLE_ASSERT(FALSE);
				return hr;
			}			
		}
	}
...
HRESULT CNLETimelineBase::OnSetRenderScale(LPCWSTR keyName, CNLEValue& oRenderScale) {
    HRESULT hr = S_OK;
    NLERational rdrScale = oRenderScale;

    NLEComPtr<INLETimelineBase> pParent = GetParentTimeline();
    //PARENT_ASSERT(pParent);
    RETURN_IF_NULL(pParent, S_OK);

    m_pProperties[keyName] = rdrScale;
    ((CNLETimelineBase *)(INLETimelineBase*)pParent)->resortClip();
    ((CNLETimelineBase *)(INLETimelineBase*)pParent)->SetRenderDirty(true);
...
HRESULT CNLETimelineBase::OnSetRenderScaleEnable(LPCWSTR keyName, CNLEValue& bRenderScaleEnable) {
    HRESULT hr = S_OK;
    INT rdrScaleEnabled = bRenderScaleEnable;

    NLEComPtr<INLETimelineBase> pParent = GetParentTimeline();
    //PARENT_ASSERT(pParent);
    RETURN_IF_NULL(pParent, S_OK);

    m_pProperties[keyName] = rdrScaleEnabled;
    ((CNLETimelineBase *)(INLETimelineBase*)pParent)->resortClip();
    ((CNLETimelineBase *)(INLETimelineBase*)pParent)->SetRenderDirty(true);
...
bool MyTimelineCompare(const TIMELINE_LIST::value_type p1, const TIMELINE_LIST::value_type p2) {
    ...
    //2. 按Position	
    INT pos1 = (CNLEValue)prop1[kRender_Position]; NLE_ASSERT(pos1 >= -1);
    INT pos2 = (CNLEValue)prop2[kRender_Position]; NLE_ASSERT(pos2 >= -1);
    if (pos1 != pos2) {
        return (pos1 <= pos2);
    }
