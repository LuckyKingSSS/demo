...
//         NLE_ASSERT(0); //we don't should use this function
	...
	NLECOMMON_API NLEComPtr<INLETimelineBase> CreateMediaTimelineFromPath(LPCWSTR pFullFilePath, INLEProperties *pPropSource /*= NULL*/)
	{
		...
		do
		{
			...
			BREAK_IF_FAILED(hr);
			NLEPropDecorator propMedia = NLECommon::NLEGetProperties(pMedia); NLE_ASSERT(propMedia);
		...
		if (NULL == ret)
		{
			...
			if (pPropSource)
			{
				...
				hr = pTimelineBase->QueryInterface(IID_INLEService, (void**)&pService); NLE_ASSERT(SUCCEEDED(hr));

				hr = pService->SetReconstructProperties(pPropSource); NLE_ASSERT(SUCCEEDED(hr));
			}
	 ...
	 NLECOMMON_API NLEComPtr<IThreadPool> GetThreadPool()
	{
		if (g_pPool == NULL)
		{
			HRESULT hr = NLECommon::GetInterface(CLSID_ThreadPool, NULL, 0, IID_IThreadPool, (LPVOID*)&g_pPool); NLE_ASSERT(SUCCEEDED(hr));
