...
HRESULT __stdcall CNLEAudioRenderBase::Prepare(INLEFrame* pGraphics)
{
	...
	do
	{
		...
		m_pMixerFrame = this->GetMixerFrame(TRUE); NLE_ASSERT(m_pMixerFrame);
		...
		NLE_ASSERT(m_pRenderFrame->GetDataSize() == m_nNextFrameSize);
		NLE_ASSERT(m_pMixerFrame->GetDataSize() == m_nNextFrameSize);
		NLE_ASSERT(pGraphics->GetDataSize() == m_nNextFrameSize);
...
HRESULT __stdcall CNLEAudioRenderBase::Render(INLEFrame* pGrahics)
{
	...
	do
	{
		...
		NLE_ASSERT(pGrahics->GetDataSize() == m_nNextFrameSize);
...
HRESULT CNLEAudioRenderBase::PrepareInternal(INLEFrame** ppOutFrame)
{
	...
	if (FAILED(hr) || pOper->GetBufferSize() < m_nNextFrameSize)
	{
		...
		if (NULL == m_pInternalFrame)
		{
			...
			NLE_ASSERT(FALSE); return E_OUTOFMEMORY;
		}
	}

	pOper = NULL;
	hr = QUERY_INTERFACE(m_pInternalFrame, IID_INLEFrameOperate, &pOper); NLE_ASSERT(pOper);
...
NLEComPtr<INLEFrame> CNLEAudioRenderBase::GetMixerFrame(BOOL bCreateIfNotExists /*= TRUE*/)
{
	...
	m_nNextFrameSize = DecideNextFrameRenderSize(); NLE_ASSERT(m_nNextFrameSize);
	...
	QUERY_INTERFACE(m_pMixerFrame, IID_INLEFrameOperate, &pFrmOperator); NLE_ASSERT(pFrmOperator);
...
{
	NLEComPtr<INLEFrameOperate> pFrmOperator = NULL;
	QUERY_INTERFACE(pGraphics, IID_INLEFrameOperate, &pFrmOperator); NLE_ASSERT(pFrmOperator);
...
HRESULT CNLEAudioRenderBase::SetDataSize(NLEComPtr<INLEFrame> pFrame, INT nDataSize, BOOL bReallocatedIfOverflow /*= FALSE*/)
{
	...
	NLE_ASSERT(nDataSize < (INT)pFrameControl->GetBufferSize());
