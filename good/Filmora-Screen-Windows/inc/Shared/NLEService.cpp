...
HRESULT __stdcall CNLEService::GetValue(LPCWSTR keyName, CNLEValue& val)
{
	...
	{
		LOGGER_ERROR_(L"Failed to get property :%s", keyName); NLE_ASSERT(FALSE);
		return hr;
	}	
	DoneGetProperty(keyName);
	return S_OK;
...
HRESULT __stdcall CNLEService::DelValue(LPCWSTR keyName)
{
	...
	{
		LOGGER_ERROR_(L"Failed to delte property:%s", keyName); NLE_ASSERT(FALSE);
		return hr;
	}
...
HRESULT __stdcall CNLEService::GetProperties(INLEProperties **ppProperties)
{
	if (ppProperties)
	{
		...
		NLE_ASSERT(SUCCEEDED(hr) && *ppProperties);
		return hr;
	}
	else
	{
...
static const LPCWSTR arrPreferKey[] =
{
	...
	kStrFileFullPath,			// 设置文件路径之后，会有一套按source出来的参数
    kSource_RangeFrameNumber,   // 设定源的范围
	kRender_Scale,				// 再改变变速
	kRender_Framerate,			// 再改变帧率{与kRender_Scale的位置可以互换}
	kRender_Position,			// 设定在父片段里的位置

    NLEKey::Freeze::kFreeze_Position,
    NLEKey::Freeze::kFreeze_Scale,
    NLEKey::Freeze::kFreeze_Enabled,
};
static const int nPreferKeyCount = sizeof(arrPreferKey) / sizeof(arrPreferKey[0]);

///需要剔除的属性表
static const LPCWSTR arrRemoveKey[] =
{
    kRender_RangFrameNumber,	// 渲染范围由kSource_RangeFrameNumber推导出来
};
static const int nRemoveKeyCount = sizeof(arrRemoveKey) / sizeof(arrRemoveKey[0]);
...
HRESULT __stdcall CNLEService::SetReconstructProperties(INLEProperties *pProperties)
{
    ...
    //剔除不需要反序列化的属性
    for (int i = 0; i < nRemoveKeyCount; i++)
    {
        LPCWSTR pKeyName = arrRemoveKey[i];
        pPropCopy->Delete(pKeyName);
    }

	//先设置这几个属性，因它们有先后顺序
	for (int i = 0; i < nPreferKeyCount; i++)
	{
